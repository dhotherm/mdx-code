...
class CodexBackend(Backend):
    ...
    async def get_info(self) -> BackendInfo:
        """Get Codex CLI version and auth status."""
    ...
    async def execute(self, task: str, cwd: Path) -> AsyncIterator[str]:
        """
        Execute a task via Codex CLI and stream output.

        Spawns: codex --quiet --approval-mode full-auto "task"
        Streams stdout line-by-line as it arrives.
        """
        proc = await asyncio.create_subprocess_exec(
            self.cli_command, "--quiet", "--approval-mode", "full-auto", task,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE,
            cwd=str(cwd),
        )
            ...
            if stderr_text:
                if "auth" in stderr_text.lower() or "login" in stderr_text.lower():
                    yield (
                        ...
                        "Run `codex` directly to sign in.\n"
                    )
                else:
                    yield f"\n[MDx Code] Backend error (exit {proc.returncode}): {stderr_text}\n"

    execute.__doc__ = """Execute a task via Codex CLI and stream output."""
    ...
    async def health_check(self) -> HealthStatus:
        """Check if Codex CLI responds to a version command."""
        if not await self.is_available():
    ...
    def _parse_output(
        ...
        """
        Parse Codex CLI output for metadata.
