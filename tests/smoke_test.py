...
class SmokeTestRunner:
    ...
    def test_version_flag(self) -> None:
        ...
        self.add_result(TestResult(
            name="--version flag",
            passed=passed,
            message=f"Expected 'MDx Code' in output, got: {result.stdout[:80]}" if not passed else "OK",
            duration_ms=ms,
            command="mdx --version",
    ...
    def test_setup(self) -> None:
        ...
        self.add_result(TestResult(
            name="mdx setup",
            passed=passed,
            message=f"Exit code {result.returncode}, expected backend listing" if not passed else "OK",
            duration_ms=ms,
            command="mdx setup",
            stdout=result.stdout,
            stderr=result.stderr,
    ...
    def test_compliance(self) -> None:
        ...
        passed = result.returncode == 0 and ("SOC" in result.stdout or "Compliance" in result.stdout)
        self.add_result(TestResult(
            name="mdx compliance",
            passed=passed,
            message=f"Exit code {result.returncode}" if not passed else "OK",
    ...
    def test_review_no_args(self) -> None:
        ...
        passed = result.returncode in (0, 1) and ("Specify" in result.stdout or "review" in result.stdout.lower())
        self.add_result(TestResult(
    ...
    def test_invalid_backend(self) -> None:
        ...
        passed = result.returncode != 0 and ("not available" in result.stdout.lower() or "not available" in result.stderr.lower())
        self.add_result(TestResult(
    ...
    def test_import_mdxcode(self) -> None:
        ...
        result, ms = self.run_command(
            [sys.executable, "-c", "import mdxcode; print('OK')"]
        )
